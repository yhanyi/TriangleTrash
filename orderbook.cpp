...
double OrderBook::getLowestAskPrice() const {
  return asks.empty() ? std::numeric_limits<double>::max()
                      : asks.begin()->first;
}
...
void OrderBook::writeOption(Option option) { options.push_back(option); }
...
void OrderBook::addOrder(Order order) {
  if (order.type == OrderType::MARKET_BUY ||
      order.type == OrderType::MARKET_SELL) {
...
void OrderBook::executeMarketOrder(Order &order) {
  if (order.type == OrderType::MARKET_BUY) {
    for (auto it = asks.begin(); it != asks.end() && order.quantity > 0;) {
      ...
      for (auto orderIt = askOrders.begin();
           orderIt != askOrders.end() && order.quantity > 0;) {
        int matchedQuantity = std::min(order.quantity, orderIt->quantity);
        executeTransaction(order.player, orderIt->player, matchedQuantity,
                           it->first);
    ...
    for (auto it = bids.rbegin(); it != bids.rend() && order.quantity > 0;) {
      ...
      for (auto orderIt = bidOrders.begin();
           orderIt != bidOrders.end() && order.quantity > 0;) {
        int matchedQuantity = std::min(order.quantity, orderIt->quantity);
        executeTransaction(orderIt->player, order.player, matchedQuantity,
                           it->first);
        order.quantity -= matchedQuantity;
        orderIt->quantity -= matchedQuantity;
        if (orderIt->quantity == 0) {
      ...
      if (bidOrders.empty()) {
        it = std::map<double, std::vector<Order>, std::greater<double>>::
            reverse_iterator(bids.erase((++it).base()));
      } else {
        ++it;
      }
    }
...
void OrderBook::executeTransaction(Player *buyer, Player *seller, int quantity,
                                   double price) {
  ...
  std::cout << "Transaction: " << buyer->name << " bought " << quantity
            << " stocks from " << seller->name << " at $" << price << " each."
            << std::endl;
}
  ...
  while (!bids.empty() && !asks.empty()) {
    ...
    if (highestBid->first >= lowestAsk->first) {
      ...
      while (!bidOrders.empty() && !askOrders.empty()) {
        ...
        // Execute the trade
        executeTransaction(bidOrder.player, askOrder.player, matchedQuantity,
                           matchPrice);
        ...
        std::cout << "Matched order: " << matchedQuantity << " @ $"
                  << matchPrice << std::endl;
      }
...
std::string OrderBook::getOrderBookDisplay() const {
  ...
  for (auto it = asks.rbegin(); it != asks.rend(); ++it) {
    ...
    // int barLength = std::min(size_sum / 10, 50);  // Cap bar length at 50
    // characters oss << std::string(barLength, '█');
    oss << "\n";
  }
...
bool OrderBook::isMarketActive() const {
  return !bids.empty() && !asks.empty() &&
         (bids.rbegin()->first > 0 || asks.begin()->first > 0);
}
