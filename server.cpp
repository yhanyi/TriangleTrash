...
void Server::createRoom(const std::string &roomCode, bool botsEnabled) {
  rooms.emplace(roomCode, Game(roomCode, 5, botsEnabled));
}
...
void Server::handleClient(int clientSocket) {
  ...
  while (true) {
    const char *roomPrompt =
        "Enter room code to join or create (or 'quit' to exit): ";
    ...
    if (!game) {
      const char *botPrompt = "Enable bots? (yes/no): ";
      send(clientSocket, botPrompt, strlen(botPrompt), 0);
      ...
      buffer[bytesRead] = '\0';
      std::string botInput(buffer);
      botInput.erase(botInput.find_last_not_of(" \n\r\t") + 1);

      bool botsEnabled = (botInput == "yes" || botInput == "y");
      createRoom(input, botsEnabled);
      game = getRoom(input);
    }

    currentRoom = input;
    game->addPlayer(
        new Player(playerName, 10000)); // Add player with initial balance
    ...
    while (true) {
      ...
      if (bytesRead > 0) {
        ...
        if (input == "help") {
          std::string helpMessage =
              ...
              "  write_call <quantity> <strike_price> <days_to_expire> - Write "
              "a call option\n"
              "  write_put <quantity> <strike_price> <days_to_expire> - Write "
              "a put option\n"
      ...
      // Update bots every 5 seconds
      auto now = std::chrono::steady_clock::now();
      if (std::chrono::duration_cast<std::chrono::seconds>(now - lastBotUpdate)
              .count() >= 5) {
        game->updateBots();
        lastBotUpdate = now;
      }
